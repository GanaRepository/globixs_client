...
export const QuickInfoSection: React.FC<QuickInfoSectionProps> = () => {
  const infoCards = [
    {
      icon: <Users size={24} className="text-globixs-primary" />,
      title: 'Expert Staff',
      description: 'Top industry professionals',
      color: 'primary',
    },
    {
      icon: <Award size={24} className="text-globixs-secondary" />,
      title: 'Proven Results',
      description: 'Delivering excellence since 2010',
      color: 'secondary',
    },
    {
      icon: <Code size={24} className="text-globixs-accent" />,
      title: 'Custom Solutions',
      description: 'Tailored to your needs',
      color: 'accent',
    },
    {
      icon: <Clock size={24} className="text-globixs-primary" />,
      title: '24/7 Support',
        ...
        <div className="flex flex-wrap justify-center">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 w-full max-w-screen-xl">
            {infoCards.map((card, index) => (
              ...
              >
                <div
                  className={`w-14 h-14 rounded-xl bg-globixs-${card.color}/10 flex items-center justify-center flex-shrink-0 group-hover:bg-globixs-${card.color} transition-colors duration-300`}
                >
                  <div
                    className={`text-globixs-${card.color} group-hover:text-white transition-colors duration-300`}
                  >
                    {card.icon}
                  </div>
                </div>
                <div>
                  <h3 className="text-lg text-globixs-dark group-hover:text-globixs-primary transition-colors duration-300">
                    {card.title}
                  </h3>
                  <p className="text-globixs-textLight text-sm mt-1">
                    {card.description}
                  </p>
                </div>
              </div>
...
export const FeaturesBanner: React.FC<FeaturesBannerProps> = () => {
  return (
    <div className="py-10 px-24 bg-gradient-to-r from-globixs-primary via-globixs-secondary to-globixs-accent animate-gradient-x bg-300% text-white">
      <div className="container mx-auto px-4">
